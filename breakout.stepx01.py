...
#
ball = Rect(WIDTH / 2, HEIGHT / 2, 24, 24)
ball.direction = 5, -5
ball.colour = "red"
...
#
BAT_LENGTH = ball.width * 4
BAT_THICKNESS = BAT_LENGTH / 10
bat = Rect(WIDTH / 2, 0.96 * HEIGHT, BAT_LENGTH, BAT_THICKNESS)
bat.colour = "green"
lbat = Rect(0, HEIGHT / 2, BAT_THICKNESS, BAT_LENGTH)
lbat.colour = "green"
rbat = Rect(WIDTH - BAT_THICKNESS, HEIGHT / 2, BAT_THICKNESS, BAT_LENGTH)
rbat.colour = "green"
...
def draw():
    ...
    * Draw the bat
    ...
    screen.draw.filled_rect(lbat, lbat.colour)
    screen.draw.filled_rect(rbat, rbat.colour)
    draw_blocks()
...
def update():
    ...
    #
    # Bounce the ball off the bottom bat
    #
    ...
    #
    # Bounce the ball off the side bats
    #
    if ball.collidelist([lbat, rbat]) >= 0:
        sounds.blip.play()
        ball.direction = -dx, dy
    ...
    #
    # If the ball hits the bottom of the screen, or either
    # side wall, you lose
    #
    if ball.bottom >= HEIGHT or ball.left <= 0 or ball.right >= WIDTH:
        sounds.die.play()
        exit()
